...
# Script to fix CORS for instructor endpoint in API Gateway
# This ensures the /instructor endpoint properly handles CORS preflight requests
...
REGION="us-east-2"
STAGE_NAME="prod"
LAMBDA_FUNCTION_NAME="pottery-instructor-handler"

echo "🔧 Fixing CORS for /instructor endpoint"
echo "======================================"
echo ""
...
# Step 2: Get or Create /instructor Resource
echo "📋 Step 2: Getting or creating /instructor resource..."
RESOURCE_ID=$(aws apigateway create-resource \
  --rest-api-id $API_ID \
  --parent-id $ROOT_RESOURCE_ID \
  --path-part "instructor" \
  --region $REGION \
  --query 'id' \
  ...
  aws apigateway get-resources \
    --rest-api-id $API_ID \
    --region $REGION \
    --query "items[?path==\`/instructor\`].id" \
    --output text)
...
aws lambda add-permission \
  ...
  --statement-id apigateway-invoke-instructor \
  --action lambda:InvokeFunction \
  --principal apigateway.amazonaws.com \
  --source-arn "arn:aws:execute-api:${REGION}:${ACCOUNT_ID}:${API_ID}/*/POST/instructor" \
...
echo "   - Resource: /instructor"
echo "   - Stage: $STAGE_NAME"
echo "   - Endpoint: https://${API_ID}.execute-api.${REGION}.amazonaws.com/${STAGE_NAME}/instructor"
echo ""
echo "✅ The /instructor endpoint should now properly handle CORS requests from https://potterychicago.com"
